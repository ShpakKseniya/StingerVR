...
{
    private OVRManager script_OVRManger;
    bool _secondaryIndexTriggerRight;
    public UnityAction OnSecondaryIndexTriggerRightHeld;
    public UnityAction OnSecondaryIndexTriggerRightReleased;
	...
	{
        Instance = this;
        script_OVRManger = GetComponent<OVRManager>();
        _secondaryIndexTriggerRight = false;
    }

	internal void OnVibrationController()
	{
    ...
    private void DisplayInputs()
    {
        ...
        if (OVRInput.Get(OVRInput.Axis1D.SecondaryIndexTrigger) > 0.35f)
        {
            ...
            if (!_secondaryIndexTriggerRight)
            {
                OnSecondaryIndexTriggerRightHeld?.Invoke();
                _secondaryIndexTriggerRight = true;
            }
        }
        else
        {
            if (_secondaryIndexTriggerRight)
            {
                OnSecondaryIndexTriggerRightReleased?.Invoke();
                _secondaryIndexTriggerRight = false;
            }
